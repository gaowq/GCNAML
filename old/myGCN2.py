...
df_features = pandas.read_csv(
    'D:\code\d4\elliptic_txs_features.csv', header=None)

# 2.数据处理、关联
df_features.columns = ['id', 'time step'] + \
    [f'trans_feat_{i}' for i in range(
        93)] + [f'agg_feat_{i}' for i in range(72)]

# 获取所有节点
all_nodes = list(set(df_edge['txId1']).union(set(df_edge['txId2'])).union(
    set(df_class['txId'])).union(set(df_features['id'])))
...
df_features = df_features[['id'] +
                          list(df_features.drop(columns=['id']).columns)]
df_features.head()
...
node_label['label'] = node_label['label'].apply(
    lambda x: '3' if x == 'unknown' else x).astype(int)-1
node_label.head()

# 合并特征数据和标签数据
merged_nodes_df = node_label.merge(df_features.rename(columns={
                                   'id': 'nid', 'time step': 'time'}).drop(columns=['time']), on='nid', how='left')
...
for i in range(49):
    #获取指定时间步长所有信息
    nodes_df_tmp = merged_nodes_df[merged_nodes_df['time']
                                   == i+1].reset_index()
    ...
    #构建图数据结构
    x = torch.tensor(numpy.array(nodes_df_tmp.sort_values(by='index').drop(
        columns=['index', 'nid', 'label'])), dtype=torch.float)
    edge_index = torch.tensor(numpy.array(
        df_edge_tmp[['source', 'target']]).T, dtype=torch.long)
...
class GCN(torch.nn.Module):
    def __init__(self, num_node_features, hidden_channels, use_skip=False):
        ...
        if self.use_skip:
            self.weight = nn.init.xavier_normal_(
...
for epoch in range(epoches):
    ...
    for data in train_loader:
        ...
        loss = criterion(out[data.train_mask],
                         data.y[data.train_mask].to(torch.int64))
    ...
    if (epoch+1) % patience == 0:
        ...
        for data in test_loader:
            data = data.to(device)
            out = model(data)
            loss = criterion(out[data.test_mask],
                             data.y[data.test_mask].to(torch.int64))
        ...
        print('Epoch: {:02d}, Train_Loss: {:.4f}, Val_Loss: {:.4f}, Precision: {:.4f}, Recall: {:.4f}, Illicit f1: {:.4f}, F1: {:.4f}'.format(
